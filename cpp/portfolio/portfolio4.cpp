...
struct stock
{
    ...
    {
        return rhs.profit * 1.0 / rhs.price / rhs.risk > profit * 1.0 / price / risk;
    }
};
...
void dfs(int cur, int remain, int prof, int risk)
{
    ...
    if (remain >= stocks[cur].price && risk >= stocks[cur].risk) {
        chosen[cur] = true;
        dfs(cur + 1, remain - stocks[cur].price,
            prof + stocks[cur].profit, risk - stocks[cur].risk);
        chosen[cur] = false;
    }
    dfs(cur + 1, remain, prof, risk);
}
